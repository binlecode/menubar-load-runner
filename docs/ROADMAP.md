...
| R7 | **A paused Keep Awake looked identical to an off one** — a suspend kills the child, so the track line went dark, and `labelMode` defaults to `.off`, making that 2pt line a default user's only ambient indicator. **Rescoped 2026-07-30** to one persistent paused tone; the transient HUD is declined below, and system notifications stay impossible (they need a bundle). **Code complete 2026-07-30:** a third, fainter tone, so the bar now means *brightness tracks the strength of the hold*; qa.sh §3e asserts the rendered tone via `LOG_AWAKE`'s `tint=`. Design record: `DESIGN-system.md` §22.5. **Perceptibility: settled, and it moved the constant.** Measured rather than eyeballed — composite the tone over a real bar (`bg×(1−α) + tint×α`) and read the contrast. The light bar is the weak case every time, since its tint is *darker* than its background: the original `0.22` gave 1.46:1 on dark but only **1.29:1** on light, near enough to an unlit bar to reinstate the bug. Shipped at **`0.30`** (1.68:1 / 1.42:1), still clearly under the foreign tone's 2.17:1 / 1.71:1 so the ordering reads. Method is in RUNBOOK §3.1; re-tune against *both* appearances or not at all. **Still open, one thing:** §3e's `tint=paused` has never actually fired — a foreign display hold correctly outranks our pause, so the case NOTEs out wherever anything holds the display. Only the precedence half is machine-confirmed; see § Verification debt. → `TODO-20260730-2129-r7-paused-keep-awake-tint.md` | P3 | A quiet machine — no display-sleep holder. |
