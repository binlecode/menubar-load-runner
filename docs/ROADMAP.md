...
|---|---|---|---|
| R5 | **Battery threshold hardwired** at 20% (`Tuning.batteryLowThresholdDefault`), no CLI/env/menu surface. A configurable threshold *relocates* the cliff — not a substitute for the arm-anyway override, which shipped. **In progress:** Steps 0–1 landed — the constant that also drove the battery trace's red band is split (`batteryChartLowThreshold` is chart-only and fixed), and the sleep path now reads a live `keepAwakeBatteryThreshold` with bounds + a clamp helper (`0` = never release on charge; the min sits above the 5% floor). No user surface yet, so behavior is still the 20% default. Remaining: CLI/env, persistence, menu surface, apply-immediately. → `TODO-20260726-2332-r5-battery-threshold-surface.md` | P3 | — |
