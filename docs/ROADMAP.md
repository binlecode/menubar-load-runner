...
| R7's `tint=paused` reading, **caught by the harness** | **Observed by hand 2026-08-01, never yet by a `qa.sh` run.** The reading itself is confirmed — `hold=partial own=0 display=0 idle=1 paused=1 tint=paused`, three consecutive ticks with no `-w` child, from a direct run of §3e case 7's own command and hook during a window when Brave dropped its `Video Wake Lock`. What remains open is only that **§3e has still never fired the case in a full run**: Brave re-acquired the lock minutes later and held it through that day's `--gui` run, which NOTEd as designed (`paused=1 tint=foreign` — the precedence half, which does pass). So the behavior is verified and the *automation* is not; don't read a NOTE here as the tone being unverified, and don't soften the case to make it pass. It needs a run that catches a machine with no display holder. Same structural gate as the readings below. |
...
**v1.19.1 and v1.19.2 are both on `origin` now** (verified 2026-08-01 with
`git ls-remote --tags origin 'v*' | sort -uV`). v1.19.2's cover changes (prose + demo, above) are still
unpublished. R7's two verification items closed 2026-08-01 — perceptibility settled at
`keepAwakeBarPausedAlpha = 0.22` by eye, and `tint=paused` observed (see § Verification debt for what that
does *not* cover).

**A new failure shape, and the sharpest one yet: a behavioral fix that no installed copy can ever be told
about.** `v1.19.2` was tagged at paused alpha `0.30`; the 2026-08-01 eyes-on decision moved it to `0.22`
one commit *after* that tag, and the tag is already published. `AppInfo.version` still reads `1.19.2`, so
`UpdateChecker` compares the highest remote tag (`v1.19.2`) against `1.19.2`, finds them equal, and offers
**no update** — while `git pull --ff-only`, the thing that actually applies an update, would deliver the
new behavior. So the fix sits on `main` reachable only by a fresh `install.sh` or an unrelated update, and
every existing install is pinned to the tone the owner rejected. Note the asymmetry that makes this class
of bug invisible: **the tag gates the prompt, the branch carries the code.** A behavioral change committed
after its release tag is therefore not "a stale tag" but an undeliverable fix, and the only resolution is a
version bump — `AppInfo.version` + CHANGELOG + README + cover badge + a new tag, per the five surfaces
above. Do not re-cut a published tag in place; `UpdateChecker` caches nothing, but a moved tag makes every
checkout's `git describe` disagree with the remote's.
