...
| Keep Awake radio-mark glyphs | Still eyes-only, but the *reason* was half wrong before 2026-07-26. `AXMenuItemMarkChar` is genuinely empty (custom `onStateImage`); "can't be screenshotted" holds only for an AX `click` — a synthesized `CGEvent` click renders the menu, which is how the Presets dot was captured for v1.14.0. That path was deliberately **not** kept as tooling: it needs the item's screen coordinates and AX reports those unreliably across displays, so it is a rebuild-ad-hoc recipe (RUNBOOK §7), not a script. Menu *structure* is now mechanical — `tests/menu-dump.applescript`. Parent title and status row *are* covered — read by index, since the ticking title makes a title-based reference go stale. |
