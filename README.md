...
Current version: **1.19.1** (see [`CHANGELOG.md`](CHANGELOG.md)).
...
While the Mac is being held awake, the label takes the same tint as the track line under the icon, so the
two read as one indicator — solid when this app is holding it, faded when something else is (a `caffeinate`
you started, another utility), since only this app's hold is one its own `Off` row can release.
