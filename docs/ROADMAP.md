...
|---|---|---|---|
| R5 | **Battery threshold hardwired** at 20% (`Tuning.batteryLowThreshold`), no CLI/env/menu surface. Not a substitute for R1's override, which shipped: a configurable threshold *relocates* the cliff. **Catch:** that constant also sets the battery trace's red band (`loadHistoryView.lowThreshold`) — split it before adding any surface, or the sparkline recolors with the sleep policy. Unblocked: `Settings ▸` and `PersistedState.Settings` both exist now. | P3 | — |
| R6 | **Fixed duration presets** (`Tuning.keepAwakeDurations` + a `Custom…` prompt). No add/remove or set-default; seconds only via CLI. Partially unblocked: the list can live in `PersistedState.Settings` with rows generated from it (add/remove by editing the file). A real add/remove/reorder UI still wants a Preferences window, which was deferred. | P4 | — |
...
| An `.app` bundle · notarization · Homebrew cask · Sparkle · a URL scheme / automation interface | **Decided 2026-07-26: stays a source-built, bundle-less binary.** Notarization is $99/yr forever for a free OSS utility, and Homebrew disables unsigned casks in the official tap on 2026-09-01, so a cask requires it — ad-hoc signing can't substitute (no stable Designated Requirement, so identity churns on every recompile, and this app recompiles on every update). Bundling would also retire the git-checkout self-update in favour of Sparkle and force a rework of the CLI-first interface, which a `.app` can't take argv for. |
...
| Keep Awake radio-mark glyphs | **Half of this was wrong, corrected 2026-07-26.** `AXMenuItemMarkChar` is indeed empty (custom `onStateImage`), but "not screenshottable" only holds for an AX `click` — a synthesized **`CGEvent`** click opens the menu *rendered*, and the Presets dot was captured that way while verifying v1.14.0. So the glyph is verifiable; nobody has done it for the tint rows yet. Needs the helper promoted out of `tmp/` first (see R13). Parent title and status row *are* covered — read by index, since the ticking title makes a title-based reference go stale. |
