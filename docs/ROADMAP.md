...
|---|---|---|---|
| R5 | **Battery threshold hardwired** at 20% (`Tuning.batteryLowThresholdDefault`), no CLI/env/menu surface. A configurable threshold *relocates* the cliff — not a substitute for the arm-anyway override, which shipped. **In progress:** Step 0 landed — the constant that also drove the battery trace's red band is split, so the sparkline can no longer follow the sleep policy (`batteryChartLowThreshold` is chart-only and fixed). Remaining: runtime value + bounds, CLI/env, persistence, menu surface, apply-immediately. → `TODO-20260726-2332-r5-battery-threshold-surface.md` | P3 | — |
