...
While Keep Awake is holding the Mac awake, the label takes the same tint as the track line under the
icon, so the two read as one indicator.
...
readers/scaler/label checks are standalone `.swift` files (`tests/readers.swift`,
`tests/scaler.swift`, `tests/label.swift`) — the closest thing to unit tests here.
