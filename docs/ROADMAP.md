...
| R13 | **Show what else is holding the Mac awake.** Sparked 2026-07-29: the menu read `Off` while two agent-owned `caffeinate -i -t 300` renewals held `PreventUserIdleSystemSleep` and this app's own window had expired an hour earlier — truthful about itself, silent about the machine. Read-only, the **eighth reader** in the same tier as the other seven: `IOPMCopyAssertionsByProcess` (IOKit `pwr_mgt`, public, headered, unprivileged), **never** parsing `pmset -g assertions` prose. On-axis rather than parity — KeepingYouAwake 1.6.8 (inspected) links no assertion-read API because its identity is a *toggle*, with nothing to read the system back for; this app is already a reader. Distinct from the declined *automation interface*, which was other processes driving this app; this is the reverse direction. **Scope, settled 2026-07-29: report the observation, never the conclusion.** The row names **owner + assertion type** and must never render as "something is holding your Mac awake" — an assertion is not an effect (`PreventUserIdleSystemSleep` alone does not hold the display, and the system follows the display down, which is why this app spawns `-di`), so that sentence can be false while the list is accurate. The founding observation proves an assertion existed, not that sleep was prevented. Two catches. (1) **The noise filter is the design work** — `powerd` holds one whenever the display is on and `WindowServer` tickles `UserIsActive` on every keystroke, so the row needs a named non-daemon owner plus a known type set, and that filter is a heuristic even though each line it shows is a fact. (2) **Renewal-based holders blink** — a 300s-renewal pattern gaps between assertions, so a 2s tick flickers without hysteresis like the speed path's. The earlier *no lever* objection is **withdrawn**: every row in this app is awareness without control, so a lever test applied here was inconsistent. | P2 | — |
...
| R13 is category-first ("no other menu-bar util tells you") | Only **KeepingYouAwake 1.6.8** was inspected — it links no assertion-read API. **Amphetamine and the standalone assertion-viewer utilities are unchecked.** Settle before the claim reaches `README.md` or `docs/cover.html`: `pmset -g assertions` is one command, so any user can check it, and a wrong category claim is worse than no claim. Does not block building R13 — only marketing it. |
